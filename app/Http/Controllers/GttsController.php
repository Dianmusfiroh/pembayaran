 ...
 */
class GttsController extends Controller
{
    /**
     * @var GttRepository
     */
     ...
     */
    public function __construct(GttRepository $repository, GttValidator $validator, InstituteRepository $instituteRepo, QualificationRepository $qualificationRepo, SkRepository $skRepo, SkDetail $sk_detailRepo, JabatanRepository $jabatanRepo,BankAccountRepository $bankRepository,KabupatenRepository $kabupatenRepo )
    {
        $this->middleware('auth');
        $this->repository = $repository;
        $this->validator  = $validator;
        $this->skRepo = $skRepo;
        $this->bankAccount = $bankRepository;
        $this->sk_detailRepo = $sk_detailRepo;
        $this->instituteRepo = $instituteRepo;
        $this->qualificationRepo = $qualificationRepo;
        $this->jabatanRepo = $jabatanRepo;
        $this->kabupatenRepo = $kabupatenRepo;
    ...
    //     // $gtts = SkDetail::whereSkId($id)->where('gtt_id','<>',null)->get();
    //     // if (request()->is('api*') || request()->ajax()) {
    //     //     $gtts = SkDetail::whereSkId($id)->where('gtt_id', '<>', null)->with(['sk','gtt'])->get();
  ...
  public function create()
    {
        ...
        $kabupaten = $this->kabupatenRepo->all();
        $bank = $this->bankAccount->all();
        ...
        // return redirect('apps/gtt/'.$sk->id.'/create');
        // return view('gtt.create', compact('modul', 'institutes', 'qualifications', 'jabatans', 'sk_id'));
        return view('gtt.create', compact('modul', 'institutes', 'qualifications', 'jabatans','bank'));
    }
    /**
    ...
    public function store(GttCreateRequest $request)
    {
        try {
            ...
            // $input['gtt_id']= $gtt->id;s
            $bank = $this->bankAccount->create($input);
    ...
    public function show($id)
    {
        ...
        return view('gtt.show', compact('gtt','modul'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('gtt.edit', compact('gtt', 'modul', 'institutes', 'qualifications', 'sks', 'jabatans', 'skdetails'));
        // , 'sk_id'
    }
    ...
    public function update(GttUpdateRequest $request, $id)
    {
        try {
            ...
            // $bank = $this->bankAccount->updateOrCreate(
            //     [
            ...
            $bankfind = $this->bankAccount->findByField('gtt_id',$id)->first();
            $bank = [
                "bank_name" => $request->bank_name,
                "account_name" => $request->account_name,
                "account_number" => $request->account_number
    ...
    public function destroy($id)
    {
        ...
        // $deleted = SkDetail::whereGttId($id)->delete();
        ...
        if (request()->searchFields != 'id:='){
            ...
            $gtts = $this->repository->scopeQuery(function ($query) use($invoice_id) {
                return $query->whereNotIn('id',function($q) use($invoice_id){
                    ...
                    return $q->select('gtt_id')->from('kinerja')->get();
                })->limit(10);
            ...
            $gtt = $this->repository->with(['kinerja'])->find($search);
            $nilai_bayar = 0;
