...
<div class="float-right">
    <a href="{{ url('master/wilayah/kabupaten/create') }}" class="btn btn-primary btn-sm"><i
...
<table class="table table-bordered table-striped table-sm text-center" id="myTable">
    ...
    <tbody>
        @foreach ($kabupatens as $key => $item)
        <tr>
            <td>{{ ($key+1) }}</td>
            <td>{{ $item->province->name }}</td>
            <td>{{ $item->name }}</td>
            <td>
                <a href="{{ url('master/wilayah/kabupaten/' . $item->id . '/edit') }}" class="btn btn-success btn-sm"
