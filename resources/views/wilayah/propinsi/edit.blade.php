...
@section('form-create')
    <form action="{{ route('provinsi.update',$province->id)}}" method="POST">
        @csrf
        @method('PUT')
@endsection
@section('card-body')
    <!-- <h3 class="title text-left">Nama Provinsi</h3> -->
    <input type="text" name="name" id="name" class="form-control" placeholder="Masukan Nama Provinsi" value = "{{$province->name}}">
@endsection