...
{{-- <div class="float-right">
    <a href="{{ {{ url('pengaturan/sumber-anggaran/' . $item->id . '/edit') }} }}" class="btn btn-primary btn-sm"><i
...
<table class="table table-bordered table-sm text-center" id="myTable">
    ...
    <tbody>
        ...
        <tr>
            <td>{{ ($key+1) }}</td>
            <td><?=str_replace('_', ' ', $item->label)?></td>
            <td>{{ $item->nilai }}</td>
            <td>
                <a href="{{ url('pengaturan/umum/' . $item->id . '/edit') }}" class="btn btn-success btn-sm"
